...
def train_LSTM(batch_size = 5,
    ...
    writer_filename = 'LSTM_na_{h}_lr_{l:.0E}_hs_{e:03d}_bs_{s:03d}'.format(
    ...
    with tf.variable_scope('RNN', reuse=None):
        ...
        for key in net_arch:
            with tf.variable_scope('{}'.format(key)):
                cell[key] = tf.contrib.rnn.BasicLSTMCell(hidden_size, state_is_tuple=False)

        state = {key:tf.zeros([batch_size,cell[key].state_size],dtype=tf.float64) for key in net_arch}
