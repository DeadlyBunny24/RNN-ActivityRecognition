...
def generate_dataset(train,
    ...
    if train:
        tfrecord_filename = os.path.join('tfRecordFiles', 'train_pos_{}'.format(num_classes) + '.tfrecords')
    else:
        tfrecord_filename = os.path.join('tfRecordFiles', 'test_pos_{}'.format(num_classes) + '.tfrecords')
